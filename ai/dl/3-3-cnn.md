...
![](fig/3-dl/cnn/5-cnn-classify.png)
...
2016 年，ResNet（残差网络）的出现进一步推动了深度神经网络的发展。下图画出了它的示意图。

![](fig/3-dl/cnn/9-resnet.png)

如上图所示，ResNet 引入了 "Skip Connection"（跳跃连接），也就是从输入直接连一根线到输出。通过这种方式，输入信息和梯度可以快速传播到网络的更深层，从而有效解决了梯度消失的问题。
